...
import {
  EditPointAddressDto,
  EditPointCoordinateDto,
  EditPointWorkingModeDto,
} from './dto/points-edit.dto';
import { DeletePointsDto } from './dto/points-delete.dto';

@Injectable()
export class PointsService {
  constructor(
    @InjectModel(PointModel) private pointsRepository: typeof PointModel,
  ) {}
  ...
  public async setPointAddress(
    dto: EditPointAddressDto,
  ...
  public async setPointCoordinate(
    dto: EditPointCoordinateDto,
    ...
    const coordinateArray: number[] = [
      pointModelById.coordinateX,
      pointModelById.coordinateY,
    ];
  ...
  public async setWorkingMode(
    dto: EditPointWorkingModeDto,
  ): Promise<IEditPointWorkingModeResponse> {
    const pointModelById = await this.getPointModelById(dto.id);
  ...
  public async deletePoints(
    ...
    if (dto.idPointsForDelete.length > 1) {
      if (!this.checkIdForDeleteValid(dto.idPointsForDelete)) {
        throw new HttpException(
          'Нельзя удалить point c id = 1',
          HttpStatus.BAD_REQUEST,
        );
      }
      const pointsForDelete = allPointsModel.filter((pointModel) => {
        const pointIdForDelete: number = dto.idPointsForDelete.find(
          (id: number) => pointModel.id === id,
        );
    ...
    if (dto.idPointsForDelete.length === 1) {
      ...
      const pointForDelete = allPointsModel.find(
        (pointModel) => pointModel.id === dto.idPointsForDelete[0],
      );
