...
export class PhonesService {
  constructor(
    @InjectModel(PhoneModel) private phonesRepository: typeof PhoneModel,
  ) {}
  ...
  public async deletePhone(dto: DeletePhoneDto): Promise<HttpException> {
    if (dto.id === 1) {
      throw new HttpException(
        'Нельзя удалить номер телефона с id = 1',
  ...
  private mapPhonesModelToPhones(phonesModel: IPhoneModel[]): IPhone[] {
    return phonesModel.map((phoneModel) =>
      this.mapPhoneModelToPhone(phoneModel),
    );
  }
