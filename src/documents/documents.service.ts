...
import {
  IDeleteDocumentsResponse,
  IDocument,
  IEditDocumentNameResponse,
...
export class DocumentsService {
  constructor(
    ...
    private documentsRepository: typeof DocumentModel,
  ) {}

  public async getAllDocuments(): Promise<IDocument[]> {
    const allDocumentsModel: DocumentModel[] =
      await this.documentsRepository.findAll();
    const response: IDocument[] =
      this.mapDocumentsModelToDocuments(allDocumentsModel);
    return response;
  }
  ...
  public async setDocumentName(
    dto: EditDocumentNameDto,
  ): Promise<IEditDocumentNameResponse> {
  ...
  public async deleteDocuments(
    dto: DeleteDocumentsDto,
  ): Promise<IDeleteDocumentsResponse[]> {
    const response: IDeleteDocumentsResponse[] = [];

    for (let i = 0; i < dto.ids.length; i++) {
      const documentModel: DocumentModel = await this.getDocumentById(
        dto.ids[i],
      );
  ...
  private mapDocumentsModelToDocuments(
    documentsModel: DocumentModel[],
  ): IDocument[] {
    return documentsModel.map((documentModel: DocumentModel) =>
      this.mapDocumentModelToDocument(documentModel),
    );
  }
