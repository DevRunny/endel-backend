...
export class CategoryService {
  constructor(
    ...
    private categoriesRepository: typeof CategoryModel,
  ) {}
  ...
  public async selectCategories(
    dto: SelectCategoriesDto,
  ...
  public async getAllCategories(): Promise<ICategory[]> {
    const categoriesModel: CategoryModel[] =
      await this.categoriesRepository.findAll();
    const response: ICategory[] =
      this.mapCategoriesModelToCategories(categoriesModel);
    return response.sort((a, b) => a.id - b.id);
  }

  public async getSelectedCategories(): Promise<ICategory[]> {
    const allCategories = await this.getAllCategories();
    const response: ICategory[] = allCategories.filter(
      (category: ICategory) => category.selected,
    );
    return response.sort((a, b) => a.id - b.id);
  }
  ...
  private mapCategoriesModelToCategories(
    categoriesModel: CategoryModel[],
  ): ICategory[] {
    return categoriesModel.map((categoryModel: CategoryModel) =>
      this.mapCategoryModelToCategory(categoryModel),
    );
  }
